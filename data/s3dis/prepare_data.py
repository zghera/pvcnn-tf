  ...
  for area_idx in range(1, 7):
    ...
    for dataset_idx, dataset in enumerate(datasets):
      ...
      if os.path.exists(dataset_marker):
        print(
          f"{datetime.now()}-{folder}/{dataset} already processed, skipping"
        )
        continue
      ...
      for offset_name, offset in offsets:
        ...
        print(
          f"{datetime.now()}-Computing block id of {xyzrgb.shape[0]} points..."
        )
        ...
        print(
          f"{datetime.now()}-{dataset} is split into {blocks.shape[0]} blocks."
        )
        ...
        for block_idx in range(blocks.shape[0]):
          ...
          for x, y in nbr_block_offsets:
            ...
            break
        print(
          f"{datetime.now()}-{block_merge_count} of {blocks.shape[0]} blocks are merged."
        )
        ...
        for block_idx in range(idx_last_non_empty_block + 1):
          ...
          xyz_grids = np.floor(
            (block_points - block_min) / args.grid_size
          ).astype(np.int)
          ...
          grid_point_indices = np.split(
            np.argsort(point_grid_indices), np.cumsum(grid_point_counts[:-1])
          )
        ...
        for block_idx in range(idx_last_non_empty_block + 1):
          ...
          block_split_num = int(
            math.ceil(block_point_num * 1.0 / max_num_points)
          )
          point_num_avg = int(
          ...
          block_xyzrgb = np.concatenate(
            [x, y, z, block_rgb, norm_x, norm_y, norm_z], axis=-1
          )
          for block_split_idx in range(block_split_num):
            ...
            indices_split_to_full[idx_in_batch, 0:point_num] = point_indices[
              start:end
            ]
            ...
            if ((idx + 1) % batch_size == 0) or (
              ...
              filename_h5 = os.path.join(
                folder, dataset, f"{offset_name}_{idx_h5:d}.h5"
              )
              ...
              if args.save_ply:
                ...
                filepath_rgb_ply = os.path.join(
                  folder, dataset, "ply_rgb", f"rgb_{offset_name}_{idx_h5:d}"
                )
                save_ply_color_batch(
                  data[0:item_num, :, 0:3],
                  data[0:item_num, :, 3:6] * 255,
                  filepath_rgb_ply,
                  data_num[0:item_num, ...],
                )
...
def save_ply(points, filename, colors=None, normals=None):
  vertex = np.core.records.fromarrays(
    points.transpose(), names="x, y, z", formats="f4, f4, f4"
  )
  n = len(vertex)
  ...
  if normals is not None:
    vertex_normal = np.core.records.fromarrays(
      normals.transpose(), names="nx, ny, nz", formats="f4, f4, f4"
    )
  ...
  if colors is not None:
    vertex_color = np.core.records.fromarrays(
      colors.transpose() * 255, names="red, green, blue", formats="u1, u1, u1"
    )
...
def save_ply_property(
  points, property, property_max, filename, cmap_name="tab20"
):
...
def save_ply_color_batch(
  ...
  for batch_idx in range(batch_size):
    ...
    if isinstance(file_path, (list, tuple)):
      ...
      save_ply(
        points_batch[batch_idx][:point_num],
        f"{basename}_{batch_idx:04d}{ext}",
        colors_batch[batch_idx][:point_num],
      )
...
def save_ply_property_batch(
  ...
  if not isinstance(file_path, (list, tuple)):
    ...
    ext = ".ply"
  property_max = (
    np.max(property_batch) if property_max is None else property_max
  )
  for batch_idx in range(batch_size):
    point_num = (
      points_batch.shape[1] if points_num is None else points_num[batch_idx]
